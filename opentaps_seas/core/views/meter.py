...
class MeterFinancialValueTable(Table):
    meter_value_id = Column(
        verbose_name="#", linkify=lambda record: record.get_absolute_url()
    )
...
class MeterFinancialValueItemTable(Table):
    ...
    class Meta:
        ...
        fields = ("from_datetime", "thru_datetime", "description", "amount")
        order_by = "-from_datetime"
    ...
    def get_breadcrumbs(self, context):
        ...
        if self.kwargs["meter"]:
            b.append(
                {
                    "url": reverse(
                        "core:meter_detail", kwargs={"meter_id": self.kwargs["meter"]}
                    ),
                    "label": "Meter {}".format(self.kwargs["meter"]),
                }
            )
        b.append(
            {"label": "Meter Financial Value {}".format(self.kwargs["meter_value_id"])}
        )
        return b
...
def meter_production_data_json(request, meter):
    ...
    if model_id:
        ...
        qs0 = qs0.filter(
            Q(**{"meter_production_reference__{}".format("BaselineModel.id"): model_id})
        )
    ...
    start, end = utils.get_start_date_from_range(
        request.GET.get("range"), from_datetime=from_datetime
    )
    if start:
    ...
    for ref in qs0.distinct("meter_production_reference").values(
        "meter_production_reference"
    ):
        ref = ref.get("meter_production_reference")
        qs = qs0.filter(meter_production_reference=ref)
        logging.info(
            "meter_production_data_json: for meter %s ref %s has %s", m, ref, qs.count()
        )
        data_ref = None
        ...
        for data in qs.order_by("-from_datetime"):
            ...
            meter_data.append({"datetime": datetime, "value": value})
    ...
    uom_d = None
    if uom:
        uom_d = {"id": uom.uom_id, "unit": uom.unit}
...
def meter_financial_value_data_json(request, meter):
    ...
    for ref in qs0.distinct("meter_production_reference").values(
        ...
        qs = qs0.filter(meter_production_reference=ref)
        logging.info(
            "meter_financial_value_data_json: for meter %s ref %s has %s",
            m,
            ref,
            qs.count(),
        )
...
def meter_data_json(request, meter):
    ...
    if not m:
        logger.warning("No Meter found with meter_id = %s", meter)
        return JsonResponse(
            {"error": "Meter data not found : {}".format(meter)}, status=404
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["object"]:
            ...
            if meter.weather_station:
                ...
                qs = WeatherHistory.objects.filter(
                    weather_station=meter.weather_station
                )
                context["has_weather_data"] = qs.count()

            latest = meter.get_meter_data().last()
            if latest:
                context["latest_meter_date"] = latest.as_of_datetime.strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            ...
            if meter.solaredgesetting_set:
                ...
                if context["has_equipments"]:
                    ...
                    for v in qs.values("entity_id"):
                        equips.append(
                            EquipmentView.objects.get(entity_id=v["entity_id"])
                        )
                        context["is_solaredge"] = True
                    context["equipments"] = equips

            history_items = MeterRatePlanHistory.objects.filter(
    ...
    def get_form_kwargs(self, *args, **kwargs):
        ...
        try:
            if not self.request.POST:
                ...
                initial_values[
                    "weather_station"
                ] = utils.get_default_weather_station_for_site(site)
...
def utility_rates_json(request):
    ...
    if not country:
        return JsonResponse(
            {"error": "Site country not found for meter : {}".format(meter_id)}
        )

    if not address:
        return JsonResponse(
            {"error": "Site address not found for meter : {}".format(meter_id)}
        )
    ...
    meter_rate_plans = MeterRatePlan.objects.filter(
        thru_datetime__isnull=True
    ).order_by("from_datetime")
    if meter_rate_plans:
        ...
        for meter_rate_plan in meter_rate_plans:
            ...
            if meter_rate_plan.from_datetime:
                name += (
                    " (from " + meter_rate_plan.from_datetime.strftime("%Y-%m-%d") + ")"
                )
            data_item["name"] = name
            data.append(data_item)
    ...
    if request.method == "POST":
        ...
        if page_label.startswith("mrp_"):
            ...
            try:
                ...
                return JsonResponse(
                    {"error": "MeterRatePlan not found : {}".format(meter_id)}
                )
            else:
            ...
            if util_rates and len(util_rates) == 1:
                ...
                from_datetime = datetime(
                    startdate.year, startdate.month, startdate.day, tzinfo=timezone.utc
                )

                if enddate:
                    enddate = datetime.fromtimestamp(enddate)
                    thru_datetime = datetime(
                        enddate.year,
                        enddate.month,
                        enddate.day,
                        23,
                ...
                return JsonResponse(
                    {
                        "error": "Cannot get rate data for selected rate plan : {}".format(
                            page_label
                        )
                    }
                )
        ...
        for item in items:
            history_item = {
                "rate_plan_history_id": item.rate_plan_history_id,
        ...
        if rate_plan_history_id:
            try:
                rph = MeterRatePlanHistory.objects.get(
                    rate_plan_history_id=rate_plan_history_id
                )
                rph.delete()
                return JsonResponse({"success": 1})
                ...
                return JsonResponse(
                    {
                        "error": "MeterRatePlanHistory not found: {}".format(
                            rate_plan_history_id
                        )
                    }
                )
        else:
...
class MeterRatePlanHistoryDetailView(
    LoginRequiredMixin, WithBreadcrumbsMixin, DetailView
):
    model = MeterRatePlanHistory
    ...
    def get_breadcrumbs(self, context):
        ...
        b.append(
            {
                "label": "Meter Rate Plan History {}".format(
                    self.kwargs["rate_plan_history_id"]
                )
            }
        )
        return b
...
class MeterRatePlanHistoryEditView(
    LoginRequiredMixin, WithBreadcrumbsMixin, UpdateView
):
    model = MeterRatePlanHistory
    slug_field = "rate_plan_history_id"
    slug_url_kwarg = "rate_plan_history_id"
    ...
    def get_success_url(self):
        obj = self.get_object()
        mrph = MeterRatePlanHistory.objects.get(
            rate_plan_history_id=obj.rate_plan_history_id
        )
...
def meter_history_total_json(request, meter_id):
    ...
    from_date_object = from_date_object.replace(
        hour=0, minute=0, second=0, microsecond=0
    )

    thru_date_object = datetime.strptime(thru_date, "%Y-%m-%d %H:%M:%S")
    thru_date_object = thru_date_object.replace(
        hour=23, minute=59, second=59, microsecond=999999
    )

    meter_history = (
        MeterHistory.objects.filter(
            meter_id=meter_id,
            as_of_datetime__gte=from_date_object,
            as_of_datetime__lte=thru_date_object,
        )
    ...
    if not meter_history:
        return JsonResponse(
            {"error": "Cannot find meter history for meter : {}".format(meter_id)}
        )
    ...
    meter_history_total = MeterHistory.objects.filter(
        meter_id=meter_id,
        as_of_datetime__gte=from_date_object,
        as_of_datetime__lte=thru_date_object,
    ).aggregate(Sum("value"))
