...
def clean():
    ...
    print('Deleting data ...')
    cursor.execute('DELETE FROM core_geo;')
    conn.commit()
...
def import_entities(source_file_name):
    ...
    for row in reader:
        if first_row:
            ...
            try:
                cursor.execute("""INSERT INTO core_geo (geo_code_id, geo_code_type_id, geo_code_name, parent_geo_code_id, geo_name)
                    VALUES (%s, %s, %s, %s, %s)""", [geo_code_id, geo_code_type_id, geo_code_name, parent_geo_code_id, geo_name])
                counter_insert += 1
                print('-- INSERT geo: ', geo_code_id)
                conn.commit()
            except IntegrityError:
                conn.rollback()
                cursor.execute("""UPDATE core_geo SET geo_code_type_id = %s, geo_code_name = %s, parent_geo_code_id = %s, geo_name = %s
                    WHERE geo_code_id = %s""", [geo_code_type_id, geo_code_name, parent_geo_code_id, geo_name, geo_code_id])
                counter_update += 1
...
def print_help():
    print("Usage: python import_geos.py [all|seed|demo] [clean]")
