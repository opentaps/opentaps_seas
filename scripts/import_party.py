...
def import_files(which):
    ...
    dirname = dirname.replace('scripts', 'data/party')
    ...
    with connections['default'].cursor() as c:
        for row in reader:
            if first_row:
                ...
                party_external_id = row[0]
                source = row[1]

                c.execute("""SELECT party_id, party_external_id, source FROM "party_party"
                    WHERE party_external_id = %s ;""", [party_external_id])
                rows = c.fetchall()

                if len(rows) > 0:
                    for dbrow in rows:
                        c.execute("""UPDATE party_party
                            SET party_external_id = %s, source = %s
                            WHERE party_id = %s
                            """, [party_external_id, source, dbrow[0]])
                        counter_update += 1
                        print('-- UPDATE party: ', dbrow[1])
                else:
                    try:
                        c.execute("""INSERT INTO party_party
                            (party_external_id, source)
                            VALUES (%s, %s)
                            """, [party_external_id, source])
                        counter_insert += 1
                        print('-- INSERT party: ', party_external_id)

                    except IntegrityError:
                        print('-- ERROR insert party, party_external_id: ', party_external_id)
...
def print_help():
    print("Usage: python manage.py runscript import_party --script-args [all|seed|demo] [clean]")
